...
class BullishAgent(AnalyticalAgent):
    ...
    def analyze(self):
        ...
        for _, row in self.df.iterrows():
            ...
            if article:
                self.save_analysis(article.id, "Bullish" if row["bullish_signal"] else "Neutral", [], None)
...
class BearishAgent(AnalyticalAgent):
    ...
    def analyze(self):
        ...
        for _, row in self.df.iterrows():
            article = self.session.query(Article).filter_by(title=row["title"]).first()
            if article:
                self.save_analysis(article.id, "Bearish" if row["bearish_signal"] else "Neutral", [], None)
